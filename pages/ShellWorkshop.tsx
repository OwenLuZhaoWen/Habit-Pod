...
const ShellWorkshop: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      alert(t("Failed to generate shell concept. Check console."));
    } finally {
    ...
    <div className="p-8 max-w-7xl mx-auto h-screen flex flex-col">
       <header className="mb-6">
        ...
        <p className="text-slate-500 mt-2">
          {t('Design custom 3D printable shells for the Standard Core Capsule using AI.')}
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1 overflow-hidden">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-y-auto">
            <h2 className="text-lg font-semibold text-slate-800 mb-4">{t('Design Parameters')}</h2>
            
            <div className="mb-6">
                <label className="block text-sm font-medium text-slate-700 mb-2">{t('Concept Prompt')}</label>
                <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder={t("e.g. A lucky cat wearing sunglasses, or a dragon eating a burger...")}
            ...
            <div className="mb-6">
                <label className="block text-sm font-medium text-slate-700 mb-2">{t('Art Style')}</label>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
                <h4 className="text-amber-800 text-sm font-semibold mb-1">{t('Hardware Constraint')}</h4>
                <p className="text-amber-700 text-xs">
                    {t('The AI generation engine is locked to preserve the central cylindrical cavity (ø 40mm) for the HabitPod Core Capsule.')}
                </p>
            </div>
            ...
            <div className="mt-auto">
                ...
                >
                    {isGenerating ? (
                        <><Loader2 className="animate-spin" /> <span>{t('Fabricating Concept...')}</span></>
                    ) : (
                        <><Sparkles size={20} /> <span>{t('Generate Shell')}</span></>
                    )}
                </button>
            </div>
        </div>
        ...
        <div className="lg:col-span-2 overflow-y-auto pr-2 pb-20">
            <h2 className="text-lg font-semibold text-slate-800 mb-4">{t('Generated Concepts')}</h2>
            ...
            {generatedDesigns.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-96 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50">
                    ...
                    <p className="text-slate-400 font-medium">{t('No designs generated yet.')}</p>
                    <p className="text-slate-400 text-sm">{t('Enter a prompt to start creating.')}</p>
                </div>
            ) : (
                    ...
                    {generatedDesigns.map(design => (
                        <div key={design.id} className="group bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl transition-all">
                            ...
                            <div className="p-4">
                                ...
                                <p className="text-xs text-slate-400 mb-4">{t('Generated')} {new Date(design.createdAt).toLocaleTimeString()}</p>
